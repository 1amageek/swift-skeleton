...
private func handle(line: String, registry: SkeletonProjectRegistry) async -> [String: Any] {
  ...
  do {
    ...
    case "index.get_skeleton":
      ...
      let accessBoundary: AccessBoundary?
      if let access = params["access"] as? String {
        guard let parsedAccess = AccessBoundary(rawValue: access) else {
          throw SkeletonError.invalidRequest("unsupported access: \(access)")
        }
        accessBoundary = parsedAccess
      } else {
        accessBoundary = nil
      }
      let options = SkeletonRenderOptions(
        accessBoundary: accessBoundary,
        kinds: Set((params["kinds"] as? [String]) ?? []),
        headersOnly: (params["headers_only"] as? Bool) ?? false
      )
...
private func requireString(_ params: [String: Any], key: String) throws -> String {
