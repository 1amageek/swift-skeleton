...
public struct SwiftLanguageRules: LanguageRules, Sendable {
  ...
  public var propertyPattern: String {
    #"^\s*(?:(?:@[A-Za-z_][A-Za-z0-9_]*(?:\([^)]*\))?\s+)|(?:(?:public|private|internal|fileprivate|package|open|static|class|final|lazy|weak|unowned|nonisolated)(?:\(set\))?\s+))*(?:var|let)\s+([A-Za-z_][A-Za-z0-9_]*)\s*:\s*([^={]+)"#
  }
  ...
  public func parseMethodStart(from trimmedLine: String) -> MethodStart? {
    ...
    guard
      let name = TextUtilities.firstRegex(
        pattern: #"func\s+([A-Za-z_][A-Za-z0-9_]*)"#, in: trimmedLine)
    else {
      return nil
    }
