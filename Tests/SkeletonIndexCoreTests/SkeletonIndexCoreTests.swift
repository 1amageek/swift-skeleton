...
@Test("extracts Swift imports and effective access")
func extractsSwiftImportsAndEffectiveAccess() throws {
  let source = """
    import FeatureTwo
    ...
    """
  let parsed = swiftParser().parse(path: "Feature.swift", source: source)
  let block = try #require(parsed.blocks.first)

  #expect(parsed.imports.map(\.moduleName) == ["FeatureTwo"])
  #expect(block.access.effective == .exported)
  #expect(block.methods.first { $0.name == "run" }?.access.effective == .exported)
  #expect(block.methods.first { $0.name == "internalRun" }?.access.effective == .module)
  #expect(block.properties.first { $0.name == "value" }?.access.effective == .lexical)
}

@Test("duplicate nested type names do not crash access extraction")
func duplicateNestedTypeNamesDoNotCrashAccessExtraction() {
  let source = """
    struct First {
    ...
    """
  let parsed = swiftParser().parse(path: "DuplicateNames.swift", source: source)

  #expect(parsed.blocks.filter { $0.typeName == "CodingKeys" }.count == 2)
  #expect(!parsed.hasParseError)
}

@Test("applies Swift effective-access and import rules")
func appliesSwiftEffectiveAccessAndImportRules() throws {
  let source = """
    @_spi(Testing) @testable import FeatureTwo
    @_exported public import FeatureThree

    public protocol PublicProtocol {
        func inheritedRequirement()
    }

    public struct PublicConcrete {
        var internalByDefault: Int
        public private(set) var readable: Int

        private struct HiddenContainer {
            public struct CappedNested {}
        }
    }

    public extension PublicConcrete {
        func exportedByExtension() {}
    }
    """
  let parsed = swiftParser().parse(path: "Access.swift", source: source)
  let protocolBlock = try #require(parsed.blocks.first { $0.typeName == "PublicProtocol" })
  let concreteBlock = try #require(parsed.blocks.first { $0.typeName == "PublicConcrete" })
  let hiddenBlock = try #require(parsed.blocks.first { $0.typeName == "HiddenContainer" })
  let nestedBlock = try #require(parsed.blocks.first { $0.typeName == "CappedNested" })
  let extensionBlock = try #require(
    parsed.blocks.first {
      $0.typeName == "PublicConcrete" && $0.kind == .extension
    })

  #expect(parsed.imports.count == 2)
  #expect(parsed.imports[0].moduleName == "FeatureThree")
  #expect(parsed.imports[0].isReexported)
  #expect(parsed.imports[0].access == .exported)
  #expect(parsed.imports[1].moduleName == "FeatureTwo")
  #expect(parsed.imports[1].isTestable)
  #expect(parsed.imports[1].spiGroups == ["Testing"])
  #expect(protocolBlock.methods.first?.access.effective == .exported)
  #expect(
    concreteBlock.properties.first { $0.name == "internalByDefault" }?.access.effective == .module)
  #expect(concreteBlock.properties.first { $0.name == "readable" }?.access.effective == .exported)
  #expect(concreteBlock.properties.first { $0.name == "readable" }?.access.setter == .lexical)
  #expect(hiddenBlock.access.effective == .lexical)
  #expect(nestedBlock.access.effective == .lexical)
  #expect(extensionBlock.methods.first?.access.effective == .exported)
}

@Test("extracts and filters the source-declared Swift module surface")
func extractsSourceDeclaredSwiftModuleSurface() throws {
  let source = """
    public typealias Identifier = String

    public func makeIdentifier() -> Identifier {
        "value"
    }

    public var sharedValue: Int {
        1
    }

    internal func hiddenFunction() {}
    ...
    public func trailingFunction() {}
    """
  let parsed = swiftParser().parse(path: "Surface.swift", source: source)
  let protocolBlock = try #require(parsed.blocks.first { $0.typeName == "CollectionLike" })
  let enumBlock = try #require(parsed.blocks.first { $0.typeName == "Mode" })

  #expect(
    parsed.declarations.contains {
      $0.kind == .typealiasDeclaration && $0.access.effective == .exported
    })
  #expect(
    parsed.declarations.contains {
      $0.kind == .function && $0.signature.contains("makeIdentifier")
    })
  #expect(
    parsed.declarations.contains {
      $0.kind == .variable && $0.signature.contains("sharedValue")
    })
  #expect(
    protocolBlock.declarations.contains {
      $0.kind == .associatedType && $0.access.effective == .exported
    })
  #expect(
    protocolBlock.declarations.contains {
      $0.kind == .subscriptDeclaration && $0.access.effective == .exported
    })
  #expect(enumBlock.declarations.filter { $0.kind == .enumCase }.count == 2)
  #expect(enumBlock.declarations.allSatisfy { $0.access.effective == .exported })
  ...
  let rendered = SkeletonFormatter().render(
    index: index,
    options: SkeletonRenderOptions(accessBoundary: .public)
  ).text
  #expect(rendered.contains("public typealias Identifier = String"))
  #expect(rendered.contains("public func makeIdentifier() -> Identifier"))
  #expect(rendered.contains("public var sharedValue: Int"))
  #expect(rendered.contains("associatedtype Element"))
  #expect(rendered.contains("subscript(index: Int) -> Element"))
  #expect(rendered.contains("case first"))
  let enumPosition = try #require(rendered.range(of: "enum Mode")?.lowerBound)
  let trailingPosition = try #require(rendered.range(of: "trailingFunction")?.lowerBound)
  #expect(enumPosition < trailingPosition)
  #expect(!rendered.contains("hiddenFunction"))
...
func indexesAllCoreFiles() throws {
  ...
  #expect(
    !index.files.values.contains { $0.hasParseError }, "No parse errors expected in own source")
}
...
func extractsParserMethods() throws {
  ...
  #expect(
    result.text.contains(
      "struct SwiftSkeletonParser: SkeletonParser, Sendable [SkeletonSwiftParser/SwiftSkeletonParser.swift:"
    ))
...
func extractsFormatterMethods() throws {
  ...
  #expect(
    result.text.contains(
      "render(ProjectIndex, String?, SkeletonRenderOptions) -> SkeletonTextResult"))
...
func extractsMultiParamInit() throws {
  ...
  // MethodSignature has a multi-line init with access metadata
  #expect(
    result.text.contains("init(String, [String], String?, SourceRange, Bool, DeclarationAccess)"))

  // SkeletonBlock has a multi-line init with access metadata
  #expect(
    result.text.contains(
      "init(SkeletonBlockKind, String, [String], SourceRange, [PropertySignature], [MethodSignature], Bool, DeclarationAccess, [SourceDeclaration])"
    ))
}
...
func allMethodRangesValid() throws {
  ...
  let methodLines = lines.filter {
    $0.trimmingCharacters(in: .whitespaces).hasPrefix("init(")
...
func filesSortedByPath() throws {
  ...
  for header in headerLines {
    guard let bracketRange = header.range(of: "["),
      let colonRange = header.range(of: ":", range: bracketRange.upperBound..<header.endIndex)
    else {
      continue
    }
...
@Test("no parse errors in any Swift source target")
func noParseErrorsInProject() throws {
  ...
  #expect(
    diagnostics.incompleteBlocks.isEmpty, "Incomplete blocks found: \(diagnostics.incompleteBlocks)"
  )
}
...
func tupleReturnTypes() throws {
  ...
  #expect(
    result.text.contains(
      "declarationHeader(String) -> (kind: SkeletonBlockKind, typeName: String, inheritance: [String])?"
    ))
...
func queryRanksByOccurrence() throws {
  ...
  for i in 0..<(hits.count - 1) {
    ...
    if currentFile == nextFile {
      #expect(
        (hits[i].startLine ?? 0) <= (hits[i + 1].startLine ?? 0),
...
func statusFileCount() throws {
  ...
  #expect(status.filesIndexed == index.files.count)
...
func projectContextMarkers() throws {
  ...
  let method = index.files["Production.swift"]?.implementationAnalysis.methods.first {
    $0.methodName == "hidden"
  }
...
func astEvidenceAvoidsLSIFalsePositives() {
  ...
  #expect(
    !analysis.findings.contains { $0.methodName == "statusRank" && $0.reason == .noOperation })
  #expect(
    !analysis.findings.contains { $0.methodName == "diagnosticCode" && $0.reason == .noOperation })
...
func updateReplacesImplementationAnalysis() throws {
  ...
  var index = try core.build(projectRoot: projectRoot)
  #expect(
    methodLine(named: "resolve", in: core.getSkeleton(index: index).text).contains("[impl!:trap]"))
